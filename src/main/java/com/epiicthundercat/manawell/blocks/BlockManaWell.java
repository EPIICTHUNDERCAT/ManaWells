package com.darkliz.lizzymod.lizzy_mana_stuff.blocks;

import java.util.Random;

import com.darkliz.lizzymod.lizzy_mana_stuff.tileentity.TileEntityManaWell;
...
public class BlockManaWell extends Block implements ITileEntityProvider{
	...
	public void updateTick(World worldIn, BlockPos pos, IBlockState state, Random rand) //runs only on server
    {
		...
		if (manaWell instanceof TileEntityManaWell)
		{
			...
			int generatedMana = 0;
			if(fillLevel < 5) generatedMana += 1 + rand.nextInt(7); //1-7 average of 4
			else if(fillLevel == 5)
			{
				int chance;
				if(storedMana <= MANA_CAP + (MANA_CAP / 2)) 		chance = 1;  //100% (chance that mana will be generated)
				else if(storedMana <= MANA_CAP * 2) 				chance = 2;  //50%
				else if(storedMana <= MANA_CAP * 2 + MANA_CAP / 2) 	chance = 4;  //25%
				else if(storedMana <= MANA_CAP * 3) 				chance = 8;  //12.5%
				else if(storedMana <= MANA_CAP * 3 + MANA_CAP / 2) 	chance = 16; //6.25%
				else if(storedMana <= MANA_CAP * 4) 				chance = 32; //3.125%
				else if(storedMana <= MANA_CAP * 4 + MANA_CAP / 2) 	chance = 64; //1.5625%
				else 												chance = 128; //0.078125%
				if(rand.nextInt(chance) == 0) generatedMana = 1 + rand.nextInt(3); //1-3 (average of 2);
				
			}
			
			storedMana += generatedMana;
			
			//set the blockstate of the mana well to reflect how much mana is in it
			if(storedMana >= MANA_CAP) 					{ if(fillLevel != 5){ fillLevel = 5; blockChange = true;}} 
			else if(storedMana >= (MANA_CAP / 4) * 3) 	{ if(fillLevel != 4){ fillLevel = 4; blockChange = true;}} 
			else if(storedMana >= (MANA_CAP / 4) * 2) 	{ if(fillLevel != 3){ fillLevel = 3; blockChange = true;}} 
			else if(storedMana >= (MANA_CAP / 4)) 		{ if(fillLevel != 2){ fillLevel = 2; blockChange = true;}} 
			else if(storedMana >= 1) 					{ if(fillLevel != 1){ fillLevel = 1; blockChange = true;}} 
			else 										{ if(fillLevel != 0){ fillLevel = 0; blockChange = true;}}
			
			//change the blockstate if needed
			if(blockChange)
			{
				worldIn.setBlockState(pos, state.withProperty(FILL_LEVEL, fillLevel), 3); //NOTE: when the blockstate of a block is changed the associated tile entity is deleted and a new one is created.
				
				//get the tile entity at pos again (the tile entity changed so it will be a new one)
				manaWell = (TileEntityManaWell) worldIn.getTileEntity(pos);	
				
				//if the blockstate is showing a model other than the empty mana well, which means there is mana in the well, set canRelease to true
				if(fillLevel != 0)
				{
			...
			//play a sound when mana is generated; if no mana is generated drain mana from nearest player, if any
			if(generatedMana > 0) 
			{
			...
			//Attempt to attract a witch a number of times equal to the difficulty level index (0-3); if successful, stop trying
			if(fillLevel != 0)
			{
				int i = 0;
				while(i < worldIn.getDifficulty().getDifficultyId() && !this.attemptAttractWitch(worldIn, pos, storedMana, rand))
				{
					i++;
	...
	private boolean attemptAttractWitch(World worldIn, BlockPos pos, int storedMana, Random rand) {
		...
		int maxLight;
		if(storedMana >= MANA_CAP) maxLight = Math.min(8 + (storedMana - MANA_CAP) / (MANA_CAP / 2), 10);
		else maxLight = 7;
        ...
        while(!worldIn.doesBlockHaveSolidTopSurface(worldIn, spawnPos.down()) && spawnPos.getY() > 0)
        {
        	spawnPos = spawnPos.down();
        }
        y = spawnPos.getY();
        
        if(worldIn.getBlockState(spawnPos.down()).getBlock() == this) //don't spawn a witch directly above a mana well
        {
        	return false;
        }
        ...
        if (this.getCanManaWellWitchSpawnHere(worldIn, spawnPos, maxLight) && entityliving.isNotColliding())
        {
            ...
            {
        		EntityPlayer closestPlayer = worldIn.getClosestPlayer(x, y, z, 128.0D);
        		if(closestPlayer != null && !closestPlayer.isSpectator())
        		{
        			if(closestPlayer.getDistanceSq(x, y, z) > 576) //24 squared
        			{
	...
	public void onEntityCollidedWithBlock(World worldIn, BlockPos pos, Entity entityIn)
    {
		if(!worldIn.isRemote)
		{
			//do a check to make sure its a player. monsters/animals shouldn't trigger this
			if(entityIn instanceof EntityPlayer)
			{
				...
				if(this.collideTimer == 3)
				{
					...
					if(manaWell.getIsDormant())
					{	
						if(worldIn.getTotalWorldTime() > manaWell.getDormantStartTime() + 60) //3 second delay before starting to suck mana
						{
							this.drainMana(worldIn, pos, player, manaWell);
						}
					}
			...
			else if(entityIn instanceof EntityWitch)
			{
				...
				if(manaWell.getCanRelease())
				{
					...
					((EntityWitch) entityIn).addPotionEffect(new PotionEffect(Potion.regeneration.id, duration, 4));
					((EntityWitch) entityIn).addPotionEffect(new PotionEffect(Potion.resistance.id, duration, 0));
					((EntityWitch) entityIn).addPotionEffect(new PotionEffect(Potion.moveSpeed.id, duration, 4));
					
					worldIn.playSoundAtEntity(entityIn, "random.orb", 0.12F, 0.5F * ((worldIn.rand.nextFloat() - worldIn.rand.nextFloat()) * 0.7F + 1.8F));
					worldIn.playSoundAtEntity(entityIn, "random.levelup", 0.1F, 0.5F * ((worldIn.rand.nextFloat() - worldIn.rand.nextFloat()) * 0.7F + 1.8F));
					
					worldIn.setBlockState(pos, worldIn.getBlockState(pos).withProperty(FILL_LEVEL, 0), 3); //note: this deletes the tile entity and creates a new one
					
					//Get the new tile entity
					manaWell = (TileEntityManaWell) worldIn.getTileEntity(pos);
					
					manaWell.setIsDormant(true);
					manaWell.setDormantStartTime(worldIn.getTotalWorldTime()); //get the total world time (in ticks) at the time of release
	...
	public void drainMana(World world, BlockPos pos, EntityPlayer player, TileEntityManaWell manaWell)
	{
		if(player != null && !player.capabilities.isCreativeMode)
		{
			if(player.experienceTotal > 0)
			{
				...
				if(!world.isRemote) //this is a redundant check - all this stuff runs only on the server
				{
					...
					else if(!world.getBlockState(pos.up(2)).getBlock().isOpaqueCube())
					{
						worldserver.spawnParticle(EnumParticleTypes.SMOKE_LARGE, false, pos.getX() + 0.5D, pos.getY()+ 1.0D, pos.getZ()+ 0.5D, 1, 0.0D, 0.0D, 0.0D, 0.0D, new int[0]);
					}
					
					worldserver.spawnParticle(EnumParticleTypes.SPELL_INSTANT, false, player.posX, player.posY, player.posZ, 18, 0.33D, 1.33D, 0.33D, 0.06D, new int[0]);
				}
	...
	public EntityPlayer getClosestPlayerWithXP(World world, double x, double y, double z, double distance)
    {
        ...
        for (int i = 0; i < world.playerEntities.size(); ++i)
        {
            ...
            if (entityplayer1.experienceTotal > 0 && !entityplayer1.capabilities.isCreativeMode && !entityplayer1.isSpectator())
            {
	...
	public TileEntity createNewTileEntity(World worldIn, int meta)
	{
		...
		if(meta == 0) //meta will be 0 if the player placed the block; in that case the mana well should start out empty
		{
			manaWell.setStoredMana(0);
			manaWell.setCanRelease(false);
		}
    ...
    private void playManaWellFillSound(World world, BlockPos pos)
    {
        ...
        world.playSoundEffect((double)pos.getX() + 0.5D, (double)pos.getY() + 0.5D, (double)pos.getZ() + 0.5D, "random.orb", volume, pitch);
    }
    
    private void playManaWellDrainSound(World world, EntityPlayer player, BlockPos pos)
    {
    	this.playManaWellFillSound(world, pos);
        world.playSoundAtEntity(player, "random.fizz", 0.3F, 0.5F * ((world.rand.nextFloat() - world.rand.nextFloat()) * 0.7F + 1.8F));
    }
